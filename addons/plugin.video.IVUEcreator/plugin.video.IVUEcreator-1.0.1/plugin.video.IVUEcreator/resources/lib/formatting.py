import os, xbmc, xbmcvfs, xbmcgui, xbmcaddon,xbmcplugin
import re
...
def get_channel_names(channels_ini):
    ...
    try:names =  [line.rstrip('\n') for line in open(channels_ini).readlines()]
    except:names =  [line.rstrip('\n') for line in open(channels_ini, 'r', encoding='utf8').readlines()]
...
def remove_formatting(label, clean_names, id):
    ...
    if "sky" in label.lower():
        ...
        for item in items:
            if item in label and 'sport' not in label:
