...
path_to_channels = translatePath('special://profile/addon_data/script.FreeVueGuide/%s' % typeName)
timeshift = translatePath('special://profile/addon_data/script.FreeVueGuide/timezone.ini')
xml = open(path_to_channels, 'rb').read()
xml = config.ensure_str(xml)
...
def Timeshift():
    ...
    d.ok('Team FreeVueIPTV', 'Your selected channels have been shifted. They will be re-configured next time you launch the guide')
    c.close()
...
def resetTimeshift():
    ...
    if not selections:
        ...
        try: f = open(timeshift, 'w', encoding='utf-8')
        except: f = open(timeshift, 'w')
        for item in saved:
            if len(item) > 1:
                f.writelines('%s' % item)
        f.close()
        d.ok('Team FreeVueIPTV', 'Your selected channels have been reset. They will be re-configured next time you launch the guide')
        c.close()
