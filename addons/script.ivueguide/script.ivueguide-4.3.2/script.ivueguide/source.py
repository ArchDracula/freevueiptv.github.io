...
timeshift = translatePath('special://profile/addon_data/script.FreeVueGuide/timezone.ini')
path_to_settings = translatePath('special://profile/addon_data/script.FreeVueGuide')
current_db = translatePath('special://profile/addon_data/script.FreeVueGuide/program.db')
temp_db = translatePath('special://profile/addon_data/script.FreeVueGuide/temp.db')
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.FreeVueGuide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.FreeVueGuide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.FreeVueGuide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        try:
            if os.path.exists(timeshift):
                os.remove(timeshift)
            xbmc.log('[script.FreeVueGuide] Updating caches...', xbmc.LOGDEBUG)
    ...
    def getChannelINI(self):
        if not os.path.exists(translatePath('special://profile/addon_data/plugin.video.IPTVsubscriber')):
            os.makedirs(translatePath('special://profile/addon_data/plugin.video.IPTVsubscriber'))
        channelsList = self.getChannelList(False,True)
        channels = [channel.title for channel in channelsList]
        ini = xbmcvfs.File('special://profile/addon_data/plugin.video.IPTVsubscriber/custom_channels.ini','w+')
    ...
    def channelSetup(self, append=None):
        setupPath = translatePath('special://profile/addon_data/script.FreeVueGuide/resources/guide_setups')
        ...
        channelsList = self.getChannelList(False,True)
        ini = xbmcvfs.File('special://profile/addon_data/script.FreeVueGuide/resources/guide_setups/%s' % xmltvfile,'w+')
...
class XMLTVSource(Source):
    PLUGIN_DATA = translatePath(os.path.join('special://profile', 'addon_data', 'script.FreeVueGuide'))
    KEY = 'xmltv'
    LOGO_SOURCE_IVUE = 0
    LOGO_SOURCE_SUB = 1
...
#Karls custom xml import for creator
        ...
        if self.xmltvType == gType.CUSTOM_FILE_ID:
            if addon.getSetting('custom.xmltv.type') == "0":
                try:
                    ...
                    pass
                xmltvTemp = translatePath(os.path.join('special://profile', 'addon_data', 'script.FreeVueGuide', 'custom.xml'))
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[script.FreeVueGuide] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
    ...
    def parseXMLTV(self, context, f, size, logoFolder, progress_callback):
            ...
            if event == "end":
                ...
                if elem.tag == "programme":
                    ...
                    if xmltvType == '':
                        ...
                        xmltvType = ADDON.getSetting('sub.xmltv')
                    xmltvfile = xmltvType + '.ini'
                    inifile = translatePath('special://profile/addon_data/script.FreeVueGuide/resources/guide_setups/%s' % xmltvfile)
        ...
        #f = xbmcvfs.File('special://profile/addon_data/script.FreeVueGuide/program_category.ini',"wb")
