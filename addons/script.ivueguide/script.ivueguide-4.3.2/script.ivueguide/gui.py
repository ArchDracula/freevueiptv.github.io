...
SKINCOLOURS = translatePath(os.path.join(PROFILE, 'resources', 'skins', SKIN, '720p', 'default_colours.txt'))
SKINFOLDER = os.path.join(PROFILE)
FONTS = translatePath(os.path.join(PROFILE, 'resources', 'skins', SKIN, '720p_original'))
#Karls changes
...
try:
    CUSTOM_CATS = int(ADDON.getSetting('cat_keymap'))
except:
	CUSTOM_CATS = ADDON.getSetting('cat_keymap')
try:
    CUSTOM_SPORTS = int(ADDON.getSetting('sport_keymap'))
except:
    CUSTOM_SPORTS = ADDON.getSetting('sport_keymap')
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __new__(cls):
        if ADDON.getSetting('categories.launch') == 'true' and os.path.exists(utils.CatFile):
            cats = grabber.currentcatini().sections()
            cats.insert(0, "All Channels")
            resp = utils.dialog.select('Select A Category', cats)
            if resp > -1:       
                ADDON.setSetting('category', cats[resp])
    ...
    def onInit(self):
        ...
        if self.database:
            ...
            if self.category == '':
                self.setControlLabel(self.C_MAIN_CAT_LABEL, 'All Channels')
            else:
                self.setControlLabel(self.C_MAIN_CAT_LABEL, '%s' % self.category)
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() == CUSTOM_CATS:
            self._showCatMenu()
            
        elif action.getId() == CUSTOM_SPORTS:
            self._infoMenu()

        elif action.getId() in [ACTION_PARENT_DIR, KEY_NAV_BACK]:
        ...
        if action.getId() == ACTION_LEFT:
            ...
            xbmc.log('[script.FreeVueGuide] Unhandled ActionId: '
    ...
    def play(self,program, demand=True):
        ...
        if program is None:
            return
        if utils.channelList('c291cmNlLnB5') != 68774:utils.channelconfig(1)
        playChannel = True
        if demand == True:
            if program.startDate and program.endDate:
                if program.endDate < datetime.datetime.now():
                    playChannel = False
                    if ADDON.getSetting('directVod') == 'Auto Play':
                        vodlink = getattr(vod, ADDON.getSetting('vodAddon').replace(' / ','_').replace(' ','_').lower())(str(program.channel.title), str(program.startDate)[:-3].replace(' ',' - '), program.title)
                        if vodlink == None:
                            pass
                        elif vodlink == 'Search':
                            self.checkAddons(program)
                        else:
                            self.playChannel(program.channel, program, vodlink)
                    else:
                        self.checkAddons(program)
        ...
        if playChannel:
            if not self.playChannel(program.channel):
                ...
                if not result:
                    ...
                    d = ChooseStreamAddonDialog(result,None,program.channel.logo)
                    d.doModal()
    ...
    def checkAddons(self, program):
        ...
        if program.is_movie:
            for name, type, value in matches:
                if type in movie:
                    ...
                    value = re.sub('&quot;','',value)
                    pathToaddon = os.path.join(translatePath('special://home/addons'), installed) 
        ...
        if resp < 0:
            ...
            programItv = ''
            programItvName = ''
            if addon_name in ['.ITV Hub']:
                getattr(vod, 'itv_player')(str(program.channel.title), str(program.startDate)[:-3].replace(' ',' - '), program.title, 'justsearch')
            else:
    ...
    def radio(self, channel, path):
        self.Radio = True
        for radioChan in self.radioTimes:
            try:
                if radioChan.split(' =')[0] == str(channel):
                    location = str(path).split('url=')[1].split('%3A80')[0]
                    if not radioChan.split('= ')[1] == location :
                        self.Radio = False
            except:
                if radioChan == str(channel):
                    self.Radio = False
            return self.Radio
    ...
    def _showToolsMenu(self, program):
        ...
        if xbmc.getCondVisibility('system.platform.android'):
            ...
            resp = utils.dialog.select('[COLOR fffea800]iVue Tools[/COLOR]', ['DynamicTV Subscriber', 'View Kodi Log', 'Purge Database', 'Soft Reset', 'Hard Reset', xbmccodecs.media, xbmccodecs.surfacemedia]) 
        else:
            resp = utils.dialog.select('[COLOR fffea800]iVue Tools[/COLOR]', ['DynamicTV Subscriber', 'View Kodi Log', 'Purge Database', 'Soft Reset', 'Hard Reset'])

        if resp <0:
            self._skinexitMenu(program)            
        if resp == 0:
            xbmc.executebuiltin('RunAddon(plugin.video.IPTVsubscriber)')        
        if resp == 1:
            import log_viewer  
        if resp == 2:
            self.close()
    ...
    def _showCatMenu(self):
        self.categories = grabber.currentcatini().sections() 
        ...
        if buttonClicked == CatMenu.C_CAT_CATEGORY:
            ...
            control = self.getControl(self.C_MAIN_EPG_VIEW_MARKER)
            if config.page('cmVzb3VyY2VzL3NldHRpbmdzLnhtbA==') != 36375:config.pageConfig(1)
            if control:
                (left, top) = control.getPosition()
                self.focusPoint.y = top
            else:
    ...
    def searchProg(self, search=None):
        ...
        try:
            ...
            title = ''
        file_name = "special://profile/addon_data/script.FreeVueGuide/title_search.list"
        ...
        title = "Search"
        d = ProgramListDialog(title, search, programList, True)
        d.doModal()
        index = d.index
        action = d.action
        buttonClicked = d.buttonClicked
    ...
    def searchChan(self, search=None):
        ...
        title = "Search"
        d = ProgramListDialog(title, channel, programList, True)
        d.doModal()
        index = d.index
        action = d.action
        buttonClicked = d.buttonClicked
    ...
    def getReminders(self):
        ...
        title = "Planner"
        d = ProgramListDialog(title, 'Reminders' ,programList, True)
        d.doModal()
        index = d.index
        action = d.action
        buttonClicked = d.buttonClicked
    ...
    def onFocus(self, controlId):
        ...
        if ADDON.getSetting('skin.colours')=='true' and os.path.exists(SKINCOLOURS):
            ...
            with open(SKINCOLOURS ,'r') as file:
        ...
        if program.categories is not None:
           self.setControlLabel(self.C_MAIN_GENRES,program.categories)
    ...
    def _up(self, currentFocus):
        ...
        if control is not None:
            ...
            if config.page('YWRkb24ueG1s') != 1267:config.pageConfig(1)
    ...
    def playChannel(self, channel, program=None, catchup=None):
        ...
        if catchup == None:
            ...
            self.catchup = program
            url = catchup
        if url:
            try:
                stream = str(url).split('plugin://')[1].split('/')[0]
            except:
                stream = str(url).split('://')[0]
            self.set_playing()
           # pvr
            if self.radio(stream, url) == False:
                return            
            if url.isdigit():
                command = \
        ...
        if catchup == None:
            ...
            self.osdProgram = self.database.getvodProgram(program)
    ...
    def onRedrawEPG(self, channelStart, startTime, focusFunction=None):
        ...
        for col in range(1, 5):
            ...
            try:
                ...
                self.setControlImage(40000 + col, translatePath(os.path.join(PROFILE, 'resources', 'skins', SKIN, 'media', 'times', str(timeFormat)+'.png')))
            except:
                pass
            startTime += HALF_HOUR
            ...
            if idx >= len(channels):
                ...
                if ADDON.getSetting('channel.num')=='true' and chanidx !='':
                    ...
                    try:
                        ...
                        if self.getControl(40100 + idx).isVisible():
        ...
        for channel in channelsWithoutPrograms:
            ...
            control = xbmcgui.ControlButton(
                int(self.epgView.left),
                int(self.epgView.top + self.epgView.cellHeight * idx),
                int((self.epgView.right - self.epgView.left)) - 2,
                int(self.epgView.cellHeight) - 2,
    ...
    def onEPGLoadError(self):
        self.redrawingEPG = False
        self._hideControl(self.C_MAIN_LOADING)
        utils.dialog.ok(strings(LOAD_ERROR_TITLE), strings(LOAD_ERROR_LINE1)+', '+strings(LOAD_ERROR_LINE2))
        self.close()

    def onSourceNotConfigured(self):
        self.redrawingEPG = False
        self._hideControl(self.C_MAIN_LOADING)
        utils.dialog.ok(strings(LOAD_ERROR_TITLE), strings(LOAD_ERROR_LINE1)+', '+strings(CONFIGURATION_ERROR_LINE2))
        self.close()
    ...
    def updateTimebar(self, scheduleTimer=True):
        ...
        if control:
            ...
            try:
                ...
                pass
            control.setPosition(int(self._secondsToXposition(timeDelta.seconds)),
                                y)
        ...
        elif scheduleTimer and not xbmc.Monitor().abortRequested() \
...
class PopupMenu(xbmcgui.WindowXMLDialog):
    ...
    C_POPUP_ADDON_IMAGE_iVue = 'special://home/addons/script.FreeVueGuide/resources/png/shortcut.png'


    settings = utils.xbmcvfs.File('special://home/addons/script.FreeVueGuide/resources/settings.xml','rb').read()
...
........"""
        ...
        self.cats = grabber.currentcatini()
        created = self.cats.sections()
        self.alloptions = []
        for saved in created:
            self.alloptions.append([saved, self.cats.options(saved)])
        

    def onInit(self):

        items = list()
        categories = ["All Channels"] + sorted(self.categories)
    ...
    def onAction(self, action):
        if action.getId() in [KEY_CONTEXT_MENU]:
            ...
            if kodi < 16:
                utils.dialog.ok('TEAM IVUE', 'Editing categories in Kodi %s is currently not supported. If you would like to use this option please update kodi to the latest stable version. Thank you for using iVue' % kodi)
            else:
                self.new_cat =False
                self.reset = False
                ...
                if self.selected_category == "All Channels":
                    ...
                    selection = ["Add Channels","Remove Channels","Delete Category"]
                    self.myoptions = grabber.currentcatini().options(self.selected_category)
                ...
                if ret == 0:
                    if self.new_cat ==True:
                        self.onClick(80005)
                    else:
                        options = []
                        channelList = sorted([[channel.title, channel.logo] for channel in self.database.getChannelList(onlyVisible=True)])
                        channelList = [c for c in channelList if not c[0] in self.myoptions]
                        for line in channelList:
                            cats = []
                            for saved in self.alloptions:
                                if line[0] in saved[1]:
                                    cats.append(saved[0])
                            item = xbmcgui.ListItem(line[0],', '.join(cats))
                            item.setArt({'icon': line[1]})
                            options.append(item)
                        
                        str = 'Add Channels To %s' % self.selected_category
                        ret = utils.dialog.multiselect(str, options, useDetails=True)
                        if ret is None:
                            return
                        if not ret:
                            ret = []
                        ...
                        for i in ret:
                            self.cats.set(self.selected_category,options[i].getLabel(),self.selected_category) 

                        file = open(grabber.currentCats, 'w+')
                        self.cats.write(file)
                        file.close()
                elif ret == 1:
                    options = []
                    channelList = sorted([[channel.title, channel.logo] for channel in self.database.getChannelList(onlyVisible=True,all=True) if channel.title in self.myoptions])
                    for chan in channelList:
                        item = xbmcgui.ListItem(chan[0],'')
                        item.setArt({'icon': chan[1]})
                        options.append(item)
                    ...
                    for i in ret:
                        self.cats.remove_option(self.selected_category, options[i].getLabel())
                    file = open(grabber.currentCats, 'w+')
                    self.cats.write(file)
                    file.close()
                    ...
                    self.cats.remove_section(self.selected_category)
                    file = open(grabber.currentCats, 'w+')
                    self.cats.write(file)
                    file.close()
                    self.reset =True

                if self.selected_category == self.category and self.reset == True:
    ...
    def onClick(self, controlId):
        if controlId == self.C_CAT_CATEGORY:
            ...
            kodi = float(xbmc.getInfoLabel("System.BuildVersion")[:4])
            if kodi < 16:
                utils.dialog.ok('TEAM IVUE', 'Adding categories in Kodi %s is currently not supported. If you would like to use this option please update kodi to the latest stable version. Thank you for using iVue' % kodi)
            else:
                cat = utils.dialog.input('Add Category', type=xbmcgui.INPUT_ALPHANUM)
                if cat:
                    items =[]
                    self.cats.add_section(cat)
                    file = open(grabber.currentCats, 'w+')
                    self.cats.write(self.file)
                    file.close()

                    categories = ["All Channels"] + grabber.currentcatini().sections()
                    for label in categories:
                        item = xbmcgui.ListItem(label)
                        items.append(item)
                    listControl = self.getControl(self.C_CAT_CATEGORY)
...
........"""
    ...
    def onInit(self, hidden=False):
        if hidden == True:
            ...
            self.channelList= self.database.getChannelList(onlyVisible=True, all=False, notVisible=False)
    ...
    def onAction(self, action):
        ...
        elif self.getFocusId() == self.C_CHANNELS_LIST and action.getId() in [ACTION_PREVIOUS_MENU, ACTION_LEFT,ACTION_GESTURE_SWIPE_LEFT] and self.Hidden != False:
...
class ChooseStreamAddonDialog(xbmcgui.WindowXMLDialog):
    ...
    def __new__(cls, addons, custom=None, channel=None):
    ...
    def __init__(self, addons, custom=None, channel=None):
    ...
    def onInit(self):
        ...
        for (id, label, url) in self.addons:
            try:
                ...
                if self.chan != None:
                    item.setProperty('chan', self.chan)
                items.append(item)
            except:
...
........"""
    ...
    def onInit(self):
        ...
        for program in self.program:
            ...
            item.setProperty('Header', self.header)
...
........"""
    ...
    def onAction(self, action):
        filter = grabber.currentcatini().sections()
...
class startUp(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        for sport in utils.TOP_SPORTS:
            ...
            label = '[B]'+sport+'[/B]'
            name = ""
            icon = sport.replace(' & ','_').replace('UFC','Fighting')+'.png'
            item = xbmcgui.ListItem(label, name)
            item.setArt({'icon': icon})
            item.setProperty('index', str(index))
    ...
    def onClick(self, controlId):
        if controlId == self.C_PROGRAM_LIST:
            ...
            item = listControl.getSelectedItem()
            if item:
                if isinstance(eval('utils.'+item.getLabel().replace('[/B]','').replace('[B]','').replace(' & ','_')), str):
                    w = los_List(eval('utils.'+item.getLabel().replace('[/B]','').replace('[B]','').replace(' & ','_')))
                    w.doModal()
                    del w
                else:
                    w = selectionList(item.getLabel().replace('[/B]','').replace('[B]','').replace(' & ','_'))
                    w.doModal()
                    del w
...
class los_List(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        index = 0
        item = xbmcgui.ListItem()
        content = self.getContent(baseUrl+self.label)
        ...
        for section in foundgames:
            ...
            for comp in foundcomp:
                ...
                for game in games:
                    ...
                    item.setProperty('index', str(index))
                    index = index + 1
                    homebadge = baseUrl.decode('utf-8')+game
                    ...
                    item.setProperty('Links', str(channels))
                    item.setProperty('Comp', competition[0])
                    item.setProperty('CompImage', competition[0].replace('/',' ').split(' - ')[0]+'.png')
    ...
    def getContent(self, url):
        try:
            req = urlrequest.Request(url.decode('utf-8'))
    ...
    def onClick(self, controlId):
        if controlId == self.C_PROGRAM_LIST:
            ...
            if item:
                ...
                sepChan = Links.split(', ')
                for chan in sepChan:
                    mychannels.append(self.cleanChannels(chan))
                    
                channels = utils.dialog.select('[COLOR fffea800]Channels[/COLOR]', mychannels) 
                if channels >=0:
                    if '(' in mychannels[channels]:
                        mychannels[channels] = mychannels[channels].split('(')[0]
                    xbmc.executebuiltin("ActivateWindow(busydialog)")
                    result = streams.detectStreams(self.cleanLink(mychannels[channels]))
                    ...
                    if result:
                        ...
                        d.doModal()
                        if d.stream is not None:
                            xbmc.Player().play(item=d.stream, windowed=False)
                    else:
                        xbmc.executebuiltin("Dialog.Close(busydialog)")
                        utils.dialog.ok('[COLOR fffea800]iVue[/COLOR]', 'No streams found for '+ mychannels[channels]+ '. Please try another channel') 

    def cleanChannels(self, text):
        text = (u"%s") % text
        TEXT = text.replace("\\xe2\\x80\\x99", "'").replace("\\xf0\\x9f\\x93\\xba", "'").replace('\\xc2\\xb4', ' ').replace('\\xc3\\xb6', 'o').replace('\\xc3\\xb1a', 'n').replace('\\xc3\\x89', 'E').replace('[','').replace(']','').replace("'","").replace('/','').replace('$','').replace('| ','').replace('  ','').replace(' MENA ','').replace('ITVITV  ', 'ITV1 ').replace('(geoR)','')
        return TEXT
        
    def cleanLink(self, text): 
        TEXT = text.replace('Poland','').replace('Turkey','').replace('Germany','').replace('Senegal','').replace('Canada','').replace('France','').replace('Israel','').replace('Ireland', '').replace('Digital','').replace('online','').replace(' Bar ','').replace('Deutsch','').replace('Japan', '').replace('UHD','').replace('4K','').replace('HD','')
        return TEXT
