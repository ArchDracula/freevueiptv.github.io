...
ADDON = xbmcaddon.Addon(id='script.FreeVueGuide')
...
class GuideTypes(object):
    ...
    filePath = translatePath(os.path.join('special://profile', 'addon_data', 'script.FreeVueGuide', 'guides.ini'))
    ...
    def __init__(self):
        #try:
            ...
            xbmc.log('[script.FreeVueGuide] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)

            if str(ADDON.getSetting('xmltv.type_select')) == '':
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[script.FreeVueGuide] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[script.FreeVueGuide] DataItem with index %s not found' % item, xbmc.LOGDEBUG)
        return value


    def getGuideById(self, id):
        xbmc.log('[script.FreeVueGuide] Finding Guide with ID: %s' % id, xbmc.LOGDEBUG)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[script.FreeVueGuide] Found Guide with data: %s' % str(guide), xbmc.LOGDEBUG)
        return ret
...
def getKodiVersion():
    ...
    version = []
    if 'result' in jsonQuery and 'version' in jsonQuery['result']:
        version = jsonQuery['result']['version']
    return version['major']
def guideurl(guideid, streamid):
    if os.stat(streampath+"/"+base64.b64decode(guideid).decode('utf-8')).st_size != streamid: return None
    else: return os.stat(streampath+"/"+base64.b64decode(guideid).decode('utf-8')).st_size
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[script.FreeVueGuide] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
            filePath = translatePath(os.path.join('special://profile', 'addon_data', 'script.mekguide', 'settings.xml'))
            ...
            if not int(ADDON.getSetting('logos.source')) == 0 and guideId not in ['6','9','15']:
                ADDON.setSetting('logos.source', '0')
                    
            elif not int(ADDON.getSetting('logos.source')) == 1 and typeName == 'Generate Own XML': 
                ADDON.setSetting('logos.source', '1')
