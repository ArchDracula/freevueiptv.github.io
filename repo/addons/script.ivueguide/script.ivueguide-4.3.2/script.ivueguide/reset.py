...
databasePath = translatePath('special://profile/addon_data/script.FreeVueGuide')
subPath = translatePath('special://profile/addon_data/script.FreeVueGuide/resources/ini')
pyPath = translatePath('special://profile/addon_data/script.FreeVueGuide/resources/subs')
setupPath = translatePath('special://profile/addon_data/script.FreeVueGuide/resources/guide_setups')
fullPath = translatePath('special://profile/addon_data/script.FreeVueGuide')
dialog = xbmcgui.Dialog()

def SoftReset():	
    clearFiles = ["guides.ini", "addons.ini", "program.db","program_category.ini"]
...
def refreshthumbs():
	conn = sqlite3.connect(translatePath("special://database/Textures13.db"))
	try:
		with conn:
			list = conn.execute("SELECT id, cachedurl FROM texture WHERE url LIKE '%%%s%%';" % ".githubusercontent.com/totaltec2014")
			for row in list:
				conn.execute("DELETE FROM sizes WHERE idtexture LIKE '%s';" % row[0])
				try: os.remove(translatePath("special://thumbnails/" + row[1]))
				except: pass
			conn.execute("DELETE FROM texture WHERE url LIKE '%%%s%%';" % ".githubusercontent.com/totaltec2014")
			xbmc.sleep(2000)
			dialog.ok('Logo refresh complete!', 'iVue Channel logos cleared... Run Team FreeVueIPTV to receive latest channel logos.')
	except:
		pass

def refreshchannames(silent=False):
	conn = sqlite3.connect(translatePath("special://home/userdata/addon_data/script.FreeVueGuide/master.db"))
	try:
		with conn:
			...
			xbmc.sleep(1000)
			if not silent: dialog.ok('Name refresh complete!', 'iVue channel names cleared... Run Team FreeVueIPTV to receive latest channel names.')
	except:
		pass
