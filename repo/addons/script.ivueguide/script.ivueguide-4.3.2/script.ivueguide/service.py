...
try:
    ADDON = xbmcaddon.Addon('script.FreeVueGuide')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.FreeVueGuide)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception as ex:
    xbmc.log('[script.FreeVueGuide] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)